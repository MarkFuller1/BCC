...
public class MainView {
	...
	public static void BuildMainView(JFrame mainFrame, MainController controller) {
		...
		// controller.getModel().setCb_SearchClass(new
		// JComboBox(controller.getDb().getAllClasses()));
		...
		// controller.getModel().setCb_SearchProfessor(new
		// JComboBox<String>(controller.getDb().getAllProfessors()));
	...
	private static void addImage(JFrame mainFrame, MainController controller) {
		...
		try {
			image = ImageIO.read(LoginView.class.getClass().getResource("/mainImage.png"));
		} catch (IOException e) {
