...
public class RegisterView {
	 ...
	 */
	public static void BuildRegisterView(JFrame mainFrame, RegisterController controller) throws SecurityException, IOException {
		FileHandler fileHandler = new FileHandler("BCC.log", true);
		LOGGER.addHandler(fileHandler);
		LOGGER.setLevel(Level.FINEST);
		LOGGER.info("Register page loaded correctly");
		...
		controller.getPanel().add(controller.getModel().getLab_Username()).setFont(new Font("Segoe UI", Font.PLAIN, 26));		
		...
		controller.getPanel().add(controller.getModel().getLab_Password()).setFont(new Font("Segoe UI", Font.PLAIN, 26));
		...
		controller.getPanel().add(controller.getModel().getClasses_possible()).setFont(new Font("Segoe UI", Font.PLAIN, 26));
		
		// classes taken list
		controller.getModel().setClasses_taken(new JList(controller.getDb().getPossibleClasses()));
		...
		controller.getPanel().add(controller.getModel().getBut_Register()).setFont(new Font("Segoe UI", Font.PLAIN, 20));
